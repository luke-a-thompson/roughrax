...
def test_hm_sigrk3_has_fourth_order_local_remainder():
    ...
    for scale in scales:
        ...
        errors.append(float(jnp.abs(_solve_hm(path, y0) - _solve_reference(path, y0))))
...
def test_hm_sigrk3_stage_scales_are_exposed_and_validated():
    solver = HMSigRK3(rho_scale=0.75, beta_scale=1.25)
    assert solver.rho_scale == 0.75
    assert solver.beta_scale == 1.25

    for name in ("rho_scale", "beta_scale"):
...
def test_moment_completion_satisfies_order_three_conditions():
    ...
    order_three_cherry = rho**3 * jnp.einsum("ik,il,im->klm", omega, cloud, cloud)
    order_three_cherry = order_three_cherry + (
        beta[:, None, None] * rho**2 * jnp.einsum("kl,km->klm", zeta, zeta)
    )
    carrier_coefficients = jnp.transpose(level_three, (2, 0, 1)) / (
        safe_beta[:, None, None] * safe_rho
    )
    order_three_chain = (
        beta[:, None, None] * rho * jnp.transpose(carrier_coefficients, (0, 2, 1))
    )
    ...
    expected_cherry = expected_cherry + jnp.transpose(level_three, (2, 1, 0))

    assert jnp.allclose(order_one, level_one, rtol=1e-12, atol=1e-12)
    assert jnp.allclose(order_two, expected_order_two, rtol=1e-12, atol=1e-12)
    assert jnp.allclose(order_three_chain, expected_chain, rtol=1e-12, atol=1e-12)
    assert jnp.allclose(order_three_cherry, expected_cherry, rtol=1e-12, atol=1e-12)
