...
def _moment_weights(level_one, level_two, level_three):
    """Build the homogeneous quadratic-cloud weights for every root field."""
    ...
    beta = jnp.cbrt(jnp.max(jnp.abs(level_three), axis=(0, 1)))
...
def _hm_sigrk3_step(term: RoughTerm, log_signature, y0):
    ...
    carrier_displacements = jnp.tensordot(
...
class HMSigRK3(AbstractStratonovichSolver[None]):
    r"""Experimental third-order homogeneous moment-completion SigRK method.
    ...
    vector-field evaluations for a ``d``-dimensional driver. All stage
    displacements and update coefficients are at the intrinsic rough scale.
    There are no derivative quotients, opposite probes, or prescribed
    ``sqrt(h)`` and ``1/sqrt(h)`` coefficient pairs.
    ...
    def num_stages(driver_dimension: int) -> int:
        """Number of vector-field evaluations per rough step."""
    ...
    def step(self, terms, t0, t1, y0, args, solver_state, made_jump):
        ...
        y1 = _hm_sigrk3_step(rough_term, log_signature, y0)
